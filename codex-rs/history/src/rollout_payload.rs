...
pub(super) enum RolloutItemWire<'a> {
    ...
    HostObservation {
        payload: Cow<'a, HostObservationItem>,
    },
    SecurityRiskScore {
...
impl<'a> From<&'a RolloutItem> for RolloutItemWire<'a> {
    fn from(item: &'a RolloutItem) -> Self {
        match item {
            ...
            RolloutItem::HostObservation(payload) => Self::HostObservation {
                payload: Cow::Borrowed(payload),
            },
...
impl From<RolloutItemWire<'_>> for RolloutItem {
    fn from(item: RolloutItemWire<'_>) -> Self {
        match item {
            ...
            RolloutItemWire::HostObservation { payload } => {
                Self::HostObservation(payload.into_owned())
            }
