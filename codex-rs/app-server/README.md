...
- `thread/setupStatus/read` — read one connection-scoped asynchronous setup handle by `clientThreadId`; returns a tagged `pending`, `ready` (`threadId` and `hostId`), or sanitized `failed` (`code`) status. Unknown, malformed, expired, cross-connection, and cleaned handles return the same generic request error. This read does not query `thread/list`; in-process embedders use `InProcessClientHandle::thread_setup_bridge()` as the supported host adapter to register pending handles and publish ready or failed terminal states. The raw `thread_setup_status()` accessor remains available for compatibility. Worktree setup remains owned by the hosting tool bridge.
